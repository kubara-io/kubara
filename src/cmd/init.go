...
func (o *InitOptions) ensureRenovateConfig(cs *config.ConfigStore) error {
	...
	generatedConfig := renovateConfig{
		...
		Description: []string{
			...
			"enabledManagers is limited to custom.regex so kubara only enables catalog updates.",
			"Remove or extend enabledManagers to let Renovate manage other dependencies in this repository.",
		},

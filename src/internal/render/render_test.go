...
func TestTemplateFiles_TCloudPublicOpenBaoModuleUsesHelmRelease(t *testing.T) {
	...
	assert.Contains(t, openbaoMain, `ingress_paths = local.ingress_path == "/" ? ["/"] : distinct([`)
	assert.Contains(t, openbaoMain, `"/ui",`)
	assert.Contains(t, openbaoMain, `"/v1",`)
	assert.Contains(t, openbaoMain, `paths = local.ingress_paths`)
	assert.NotContains(t, openbaoMain, `paths = [local.ingress_path]`)

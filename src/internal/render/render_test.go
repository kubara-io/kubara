...
func TestTemplateFiles_TCloudPublicAgenciesUseDefaultProvider(t *testing.T) {
	...
	assert.Contains(t, infrastructureMain, `module "openbao_unseal_kms_key"`)
	assert.Contains(t, infrastructureMain, `seal \"tcloudpublickms\"`)
	assert.Contains(t, infrastructureMain, `resource "kubernetes_secret_v1" "openbao_auto_unseal"`)
	...
	assert.Contains(t, infrastructureMain, "depends_on = [module.cce_cluster, kubernetes_secret_v1.openbao_auto_unseal]")
	...
	assert.Contains(t, infrastructureVariables, `variable "openbao_auto_unseal_enabled"`)
	assert.Contains(t, infrastructureVariables, `variable "openbao_auto_unseal_secret_name"`)
	...
	assert.Contains(t, infrastructureEnv, `openbao_auto_unseal_enabled             = false`)
	assert.Contains(t, infrastructureEnv, `openbao_auto_unseal_secret_name         = "openbao-tcloudpublickms"`)
	...
	assert.Contains(t, infrastructureOutputs, `output "openbao_auto_unseal_kms_key_id"`)
...
func TestTemplateFiles_TCloudPublicOpenBaoModuleUsesHelmRelease(t *testing.T) {
	...
	assert.Contains(t, openbaoMain, `create_namespace = var.create_namespace`)
	assert.Contains(t, openbaoMain, `timeout          = var.helm_timeout`)
	assert.Contains(t, openbaoMain, `repository = var.image_repository`)
	assert.Contains(t, openbaoMain, `extraSecretEnvironmentVars = var.extra_secret_environment_vars`)
	...
	assert.Contains(t, openbaoVariables, `variable "image_repository"`)
	assert.Contains(t, openbaoVariables, `variable "extra_secret_environment_vars"`)
}

...
When `ingress_enabled` is set, the module exposes OpenBao below the configured path prefix and annotates the ingress with the expected Traefik rewrite middlewares. The middleware objects are rendered by the Traefik Helm values.

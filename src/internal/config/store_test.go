...
func TestConfigStore_LoadMigratedLegacyFileDoesNotPersistCatalogDefaults(t *testing.T) {
	configYAML := fmt.Sprintf(`
version: %s
bootstrapCatalog: %q
clusters:
...
`, "v1alpha4", *testBootstrapCatalogPtr(), testGeneralCatalogPath)
	...
	// In memory, service catalog defaults must be applied
	require.Len(t, cs.GetConfig().Clusters, 1)
	assert.Contains(t, cs.GetConfig().Clusters[0].Services, "cert-manager")

	// On disk, the auto-migrated PlatformSetup must not have baked in catalog defaults
	...
	require.NoError(t, k8syaml.Unmarshal(savedBytes, &doc))
	require.Len(t, doc.Spec.Clusters, 1)
	assert.NotContains(t, doc.Spec.Clusters[0].Services, "cert-manager")
	assert.Nil(t, doc.Spec.Clusters[0].ArgoCD.HelmRepo)
}

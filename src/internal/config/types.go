...
// ClusterNetworking supplies shared routing inputs to catalog templates.
type ClusterNetworking struct {
	Type    NetworkingType            `json:"type" yaml:"type" jsonschema:"enum=ingress,enum=gateway,default=ingress,description=Routing API used by catalog templates."`
	Ingress *IngressNetworking        `json:"ingress,omitempty" yaml:"ingress,omitempty" jsonschema:"description=Ingress routing settings."`
	Gateway *service.GatewayReference `json:"gateway,omitempty" yaml:"gateway,omitempty" jsonschema:"title=Gateway,description=Parent Gateway for routes generated by compatible catalogs."`
}

// NetworkingType selects the routing API.
type NetworkingType string

const (
	NetworkingIngress NetworkingType = "ingress"
	NetworkingGateway NetworkingType = "gateway"
)

// IngressNetworking supplies Ingress-specific routing settings.
type IngressNetworking struct {
	ClassName string `json:"className,omitempty" yaml:"className,omitempty" jsonschema:"minLength=1,default=traefik,description=Ingress class."`
}

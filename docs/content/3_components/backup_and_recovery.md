...
For T Cloud Public CCE, use an OBS endpoint and choose the backup mode explicitly:
...
With `backupStorage.create: true`, the generated T Cloud Public infrastructure layer creates the OBS bucket and access credentials. Read the `velero_credential_access_key` and `velero_credential_secret_access_key` Terraform outputs and provide them to the OpenBao layer as `TF_VAR_velero_access_key_id` and `TF_VAR_velero_secret_access_key`. Then copy `customer-service-catalog/terraform/<cluster-name>/openbao/secrets.tf-example` to `secrets.tf` and keep the `velero_credentials` block so OpenBao writes `secret/velero/velero_s3_credentials` with key `cloud`.
...
With `backupStorage.create: false`, kubara does not generate the Terraform bucket or credentials. Provide the S3-compatible credentials yourself in your secret backend using the same AWS credentials file format shown above.

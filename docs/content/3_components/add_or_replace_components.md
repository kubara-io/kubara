...
2. Next, you need to re-template your Helm charts with Kubara (kubara generate --helm).
   Retemplating will also remove the corresponding entries in your values.yaml files in the customer-service-catalog folder. For example, if you have disabled nginx ingress, all ingress directives will also be removed.
3. Now you can add the new component (see above, adding components).
   ...
   For example, if you now use Traefik as a new ingress controller, you have to set the corresponding Traefik ingress directives yourself in the values.yaml files.

...
OBS buckets generated by the shared bucket module use KMS server-side encryption by default and create the IAM policy, group, role assignment, user membership, and bucket policy needed by the generated OBS access user. The bucket policy includes bucket metadata access such as `HeadBucket`, which S3-compatible clients use to validate the bucket before reading or writing objects.
...
By default, the generated Velero values point at the Terraform-managed bucket name `velero-<cluster-name>-<stage>`, region `eu-de`, and endpoint `https://obs.eu-de.otc.t-systems.com`. If you override `velero_bucket_name` or target another OBS region in Terraform, set the matching `services.velero.config.s3BucketName`, `s3BucketRegion`, or `s3Url` values in the cluster config.
